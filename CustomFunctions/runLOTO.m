...
function [consolidated_iTimeCells_loto, diff_nTimeCells, consolidated_TI_loto, diff_TI] = runLOTO(Data, method, freqThreshold, delta, allCells, skipFrames)
...
for trial2remove = 1:size(Data,2)
    ...
    if method == 'A' %No filtering for % of trials active
    ...
    elseif method == 'B' %PSTH then filtering for % of trials active
        ...
        [cellRastor_loto, cellFrequency_loto, timeLockedCells_loto, importantTrials_loto] = ...
            getFreqBasedTimeCellList(Data, timeLockedCells_temp_loto, freqThreshold, skipFrames, delta);
        iTimeCells = find(timeLockedCells); %Absolute indexing
        fprintf('%i time-locked cells found\n', length(iTimeCells_loto))
    elseif method == 'C' %Filter for % trials active then PSTH
