function plotCell_Analysis(cell, trialPhase, cellRastor, dfbf_sigOnly, cellFrequency, AUC, MAX_value, MAX_index, importantTrials, fontSize, lineWidth)
    ...
    title(sprintf('Activity for Cell %d in %s', cell, trialPhase), 'FontSize', fontSize)
    ...
    %legend
    legend('Mean Fluorescence Activity', 'Sum of Significant Events')
    title(sprintf('Tuning Curve for Cell %d', cell), 'FontSize', fontSize)
    xlabel('Frames','FontSize', fontSize)
    ...
    ylabel('Trials','FontSize', fontSize)
    title(sprintf('Rastor for Cell %d', cell), 'FontSize', fontSize)
    xlabel('Frames','FontSize', fontSize)
