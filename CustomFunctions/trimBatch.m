...
for job = 1:length(params)
    ...
    save([saveFolder db.mouseName '_' db.date '_synthDataAnalysis_method' params(job).methodList '_batch_' num2str(params(job).sdcpStart) '-' num2str(params(job).sdcpEnd) 'run' num2str(run) '_trimmed.mat'], 'holyData', '-v7.3');
    disp('... done!')
end
toc
disp('All done!')
